...
def compute_transport_plan(f_aa, g_bb, g_ab, f_ba, C_xx, C_yy, C_xy, C_yx, a, b, epsilon, debias=True):
    """
    Compute the optimal transport plan π given dual potentials and cost matrices for a batched setup.

    Parameters:
    - f_aa, g_bb: Dual potentials within distributions a and b (only if debiasing) [bs, M], [bs, N]
    - g_ab, f_ba: Dual potentials across distributions a and b [bs, M, N], [bs, N, M]
    - C_xx, C_yy: Cost matrices within distributions a and b (only if debiasing) [bs, M, M], [bs, N, N]
    - C_xy, C_yx: Cost matrices across distributions a and b [bs, M, N], [bs, N, M]
    - a, b: Distributions a and b [bs, M], [bs, N]
    - epsilon: Regularization parameter
    - debias: Whether debiasing is enabled

    Returns:
    - pi: Optimal transport plan [bs, M, N]
    """
    #bs = a.shape[0]
    f_size = f_ba.shape[1]
    g_size = g_ab.shape[1]
    ...
    f_ba= f_ba.unsqueeze(-1).expand(-1, -1, g_size)
    g_ab= g_ab.unsqueeze(-1).expand(-1, -1, f_size).permute(0, 2, 1)


    # Exponentiate the dual potentials for inter-distribution transport
    pi = ((f_ba + g_ab - C_xy) / epsilon).exp()

    # Multiply by the original distributions
    #if a is not None and b is not None:
    #    pi = pi * a.unsqueeze(2) * b.unsqueeze(1)
